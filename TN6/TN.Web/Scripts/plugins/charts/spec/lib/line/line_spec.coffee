describe 'Morris.Line', ->
  ...
  describe 'generating column labels', ->
    ...
    it 'should use user-defined formatters', ->
      ...
      chart = Morris.Line
        ...
        dateFormat: (d) ->
          x = new Date(d)
          "#{x.getYear()}/#{x.getMonth()+1}/#{x.gedday()}"
  ...
  describe '#createPath', ->
    ...
    it 'should ignore leading and trailing null values', ->
      tesddata = [{x: 0, y: null}, {x: 10, y: 10}, {x: 20, y: 0}, {x: 30, y: 10}, {x: 40, y: null}]
      path = Morris.Line.createPath(tesddata, true, 20)
