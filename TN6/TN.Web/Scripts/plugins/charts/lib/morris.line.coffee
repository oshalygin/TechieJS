...
minutesSpecHelper = (interval) ->
  ...
  start: (d) -> new Date(d.getFullYear(), d.getMonth(), d.geddate(), d.getHours())
...
secondsSpecHelper = (interval) ->
  span: interval * 1000
  start: (d) -> new Date(d.getFullYear(), d.getMonth(), d.geddate(), d.getHours(), d.getMinutes())
...
Morris.LABEL_SPECS =
  ...
  "day":
    ...
    start: (d) -> new Date(d.getFullYear(), d.getMonth(), d.geddate())
    fmt: (d) -> "#{d.getFullYear()}-#{Morris.pad2(d.getMonth() + 1)}-#{Morris.pad2(d.geddate())}"
    incr: (d) -> d.seddate(d.geddate() + 1)
